...
Mandatory downward delegation применяется к **meaningful bounded work unit** — самостоятельной подзадаче, у которой есть собственный cognitive output, implementation result, review result или verification result и которую можно передать с проверяемым contract.

Атомарные orchestration/tool operations не обязаны превращаться в отдельного субагента только ради использования более низкой модели. Одиночное чтение файла, один targeted search/`grep`, получение status, один механический tool call или другой короткий шаг, который не образует самостоятельного work unit, может выполнить HQ напрямую. Не дроби одну логически атомарную операцию искусственно ради создания субагента.

Каждый раз, когда при decomposition или execution главный агент понимает, что meaningful bounded task либо отдельную meaningful bounded подзадачу может надёжно выполнить более низкая модель Codex из реально доступных collaboration tools, он обязан создать субагента на этой более низкой модели и передать ему такую работу вместо выполнения её на более высокой модели HQ.

Под «более низкой моделью» здесь понимается именно менее мощная / более экономичная модель семейства Codex, доступная в текущей среде и подтверждённая как lower tier по правилам model hierarchy из раздела 11. Если задачу способны надёжно выполнить несколько подтверждённых более низких моделей Codex, выбирай lowest capable из них.

Перед тем как оставить meaningful bounded work на `HQ_DIRECT`, главный агент обязан выполнить явную внутреннюю проверку: существует ли доступная подтверждённо более низкая модель Codex, которой можно безопасно и надёжно отдать эту работу. Если да — route должен быть `SUBAGENT`, а не `HQ_DIRECT`.

## Upward escalation на модель Codex выше HQ — только с разрешения owner

Higher-model escalation допускается только при наличии конкретного capability gap: HQ установил, что текущая модель не может с достаточной надёжностью выполнить acceptance criteria из-за reasoning complexity, context/capability limitation, specialized capability requirement, material risk или проверяемой неудачи разумной bounded попытки. Формулировка «более сильная модель может сделать лучше» сама по себе недостаточна.

Если такой capability gap установлен и доступна подтверждённо более высокая модель Codex, HQ может предложить создать higher-model субагента, но не имеет права создавать его автоматически.

До создания субагента на модели Codex выше модели HQ главный агент обязан запросить у owner явное разрешение. В запросе кратко укажи: конкретную bounded задачу, evidence capability gap, почему HQ недостаточно, и какую доступную более высокую модель предлагается использовать.

Approval является scoped: он действует только для указанной bounded task, указанной модели и текущей escalation. Не переноси approval автоматически на другие задачи, workstreams, модели или будущие сессии, если owner явно не дал более широкое разрешение. Отсутствие ответа или неоднозначный ответ не считается разрешением.

Передавай higher-model субагенту только **irreducible hard part**, для которого действительно нужен этот model tier: например, difficult architecture/security reasoning, сложную диагностику или другой capability-bound fragment. Mechanical implementation, inventory, formatting, routine tests и другие части возвращай на lowest capable model, если они не требуют той же высокой capability.

Пока approval не получен, продолжай всю независимую работу, которую можно безопасно выполнить без higher-model субагента. Если именно эта эскалация является единственным оставшимся путём, состояние — `NEEDS_APPROVAL`, а не `BLOCKED`.

Создавай субагентов, если хотя бы одно условие выполняется:

- meaningful bounded task может надёжно выполнить подтверждённо более низкая модель Codex;
- после явного scoped approval owner bounded task требует доступной модели Codex выше модели HQ;
...
Не создавай отдельного субагента только когда нет подходящего lower-model candidate, collaboration tools недоступны, более приоритетная инструкция запрещает делегирование, нужные инструменты недоступны субагенту либо задача настолько неотделима от текущего HQ-context, что субагент не сможет надёжно выполнить bounded contract.
...
## Model hierarchy и определение capable

Считай одну модель Codex ниже или выше другой только когда такое отношение подтверждается актуальным tool/environment metadata, официальным model guidance текущей среды или явной organizational model policy. Не выводи hierarchy только из model ID, номера версии, названия или предположения о цене. Если надёжного ordering evidence нет, не называй модель lower/higher относительно HQ и не применяй к ней автоматическое downward/upward правило до появления evidence.

Модель считается **capable** для bounded contract только если одновременно:

- ей доступны необходимые tools/capabilities;
- требуемый context помещается и может быть передан без потери material information;
- она способна выполнить acceptance criteria с требуемой надёжностью;
- expected error risk приемлем для impact задачи;
- её ограничения не требуют от HQ скрытого выполнения основной cognitive части вместо неё.

Базовый принцип model selection: используй **lowest confirmed capable Codex model**. Сначала проверь подтверждённо lower-tier модели; если подходящая есть — делегируй ей meaningful bounded work. Если lower tier недостаточен, определи, способен ли HQ выполнить задачу. Переход к подтверждённо higher-tier модели допускается только после scoped owner approval по разделу 10.

Model tier и `reasoning_effort` — разные axes. Повышение reasoning effort на той же модели не считается upward model escalation и само по себе не требует owner approval, если higher-priority rules не требуют иного. Сначала используй достаточный effort на текущей модели; переходи на другой model tier только когда effort не закрывает capability gap.

Выбирай класс модели по задаче:

- strongest capable model — сложная архитектура, security, concurrency, миграция, неоднозначный cross-repository synthesis, high-risk review или задача с дорогой ошибкой; если такая модель выше HQ, сначала требуется scoped approval owner;
...
- specialized model — только если её заявленные capabilities прямо соответствуют task и доступны в tool schema; если она выше HQ, сначала требуется scoped approval owner.
...
1. сначала прочитай актуальный список доступных моделей, capabilities, доступный ordering evidence и поддерживаемые effort levels из tool/environment;
2. установи model relation к HQ только из подтверждённого evidence, не из названия;
3. среди подтверждённых lower-tier моделей найди lowest capable model для meaningful bounded contract;
4. если такая модель есть, создай субагента именно на ней и делегируй задачу;
5. если lower tier недостаточен, попробуй достаточный reasoning effort на HQ, если это разумно для задачи;
6. если сохраняется доказанный capability gap и требуется higher-tier Codex model, запроси scoped owner approval до spawn;
7. явно укажи `model` и `reasoning_effort` при создании субагента, если интерфейс это позволяет;
8. если override недоступен или наследование родительской модели необходимо для correctness из-за context/tool constraints, выбери наследование намеренно и не утверждай конкретный override, которого runtime не гарантирует;
9. если runtime предоставляет effective model/effort metadata после spawn, проверь её; не утверждай, что задача выполнена конкретной моделью или effort, если runtime это не подтверждает;
10. не выбирай слабую модель только ради экономии, когда ошибка materially опасна;
11. не выбирай самую дорогую модель для простого file search или механического diff;
12. если выбранная модель не поддерживает нужный effort, выбери ближайшую поддерживаемую комбинацию;
13. при неудаче из-за недостаточной глубины сначала уточни contract, decomposition или повысь effort в пределах уже разрешённой модели; если следующий разумный шаг требует модели выше HQ, запроси owner approval до её использования.
...
Default maximum nesting depth: `HQ → subagent → nested subagent`. Не создавай третий уровень вложенности без явной технической необходимости и owner approval. Вложенный субагент применяет те же правила model hierarchy, scoped approval, bounded scope и non-overlapping writes. Если вложенному субагенту потребовалась модель Codex выше модели исходного HQ, такое повышение также требует предварительного явного scoped approval owner; approval нельзя обходить через цепочку вложенных агентов.

Ownership bounded task остаётся у назначенного агента до completion, явного возврата HQ или intentional redelegation. Не передавай одну и ту же задачу между агентами по кругу и не создавай agent ping-pong.
...
Не делай более двух materially similar attempts на одном model tier без новых evidence. После двух содержательно одинаковых неудачных попыток измени хотя бы одно: contract, decomposition, reasoning effort, tools или model tier. Transient tool/network failure без содержательной model attempt не считается одной из этих попыток. Не создавай бесконечные duplicate agents.
...
- scoped approval owner перед созданием субагента на модели Codex выше модели HQ;
