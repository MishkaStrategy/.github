...
Базовый принцип model selection: используй **lowest capable Codex model**. Сначала проверь, может ли задачу с необходимой надёжностью выполнить модель Codex ниже модели HQ; если может — выбери её. Переходи к более сильной модели только тогда, когда complexity, risk, required context, tool access или предыдущая проверяемая неудача показывают, что более низкой модели недостаточно.
...
4. явно укажи `model` и `reasoning_effort` при создании субагента, если интерфейс это позволяет;
5. если override недоступен или наследование родительской модели необходимо для correctness из-за context/tool constraints, выбери наследование намеренно и зафиксируй это в своей orchestration logic;
6. не выбирай слабую модель только ради экономии, когда ошибка materially опасна;
7. не выбирай самую дорогую модель для простого file search или механического diff;
8. если выбранная модель не поддерживает нужный effort, выбери ближайшую поддерживаемую комбинацию;
9. при неудаче из-за недостаточной глубины сначала уточни contract или повысь effort/model для повторной bounded попытки, а не создавай бесконечные дубликаты.
