...
Перед существенной работой главный агент обязан оценить, улучшат ли субагенты скорость, качество или независимую проверку.
...
Не создавай субагента, если задача тривиальна, строго последовательна, быстрее выполняется главным агентом или требует постоянного совместного редактирования одного scope. Не делегируй ради количества и не создавай несколько агентов для одной и той же работы без цели variance reduction.
...
- strongest capable model — сложная архитектура, security, concurrency, миграция, неоднозначный cross-repository synthesis, high-risk review или задача с дорогой ошибкой;
...
- specialized model — только если её заявленные capabilities прямо соответствуют task и доступны в tool schema.
...
2. оцени complexity, risk, context size, latency и необходимость специализации;
3. явно укажи `model` и `reasoning_effort` при создании субагента, если интерфейс это позволяет;
4. если override недоступен или наследование родительской модели лучше сохраняет необходимый контекст, выбери наследование намеренно и зафиксируй это в своей orchestration logic;
5. не выбирай слабую модель только ради экономии, когда ошибка materially опасна;
6. не выбирай самую дорогую модель для простого file search или механического diff;
7. если выбранная модель не поддерживает нужный effort, выбери ближайшую поддерживаемую комбинацию;
8. при неудаче из-за недостаточной глубины сначала уточни contract или повысь effort/model для повторной bounded попытки, а не создавай бесконечные дубликаты.
...
Субагент может создавать собственных субагентов только когда его contract и текущие higher-priority instructions это разрешают. В таком случае он применяет те же правила model selection, bounded scope и non-overlapping writes. Главный агент всё равно владеет итоговой интеграцией.
